...
def add_soltab(
    ...
    soltab_val.attrs["AXES"] = np.array(soltab_axes_str, dtype=H5_TO_C_10)
    soltab_val[:] = val
    soltab_weight = soltab.create_dataset("weight", shape=weight.shape, dtype="<f2")
    soltab_weight.attrs["AXES"] = np.array(soltab_axes_str, dtype=H5_TO_C_10)
...
def zero_terminated(size: int = 10) -> h5py.Datatype:
    tid = h5py.h5t.C_S1.copy()
    tid.set_size(size)
    return h5py.Datatype(tid)


H5_TO_C_10 = zero_terminated(10)
...
def write_rm_to_h5parm(
    ...
    with h5py.File(h5parm_name, "a") as h5parm:
        ...
        axes_values = {}
        axes_values["ant"] = np.array(station_names, dtype=H5_TO_C_10)
        axes_values["time"] = (
            rms[station_names[0]].times.mjd * 24 * 3600.0
        )  # mjd in seconds?
        axes_values["dir"] = np.array([src_name], dtype=H5_TO_C_10)
        rm_values = np.array(
